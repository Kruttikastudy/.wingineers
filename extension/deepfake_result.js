...
function displayResult(analysis) {
  ...
  if (verdict === 'DEEPFAKE') {
    ...
    color = 'deepfake';
    subtitle = 'This media shows characteristics consistent with deepfake manipulation. Exercise caution with this content.';
  } else if (verdict === 'AUTHENTIC') {
    message = 'Likely Authentic';
    color = 'authentic';
    subtitle = 'This media appears to be genuine with no obvious signs of manipulation.';
  } else {
    message = 'Inconclusive';
    color = 'inconclusive';
    subtitle = 'The analysis is inconclusive. The media may be authentic or manipulated. Human review recommended.';
  }
...
function populateDetails(details) {
  ...
  if (details.frames_analyzed !== undefined) {
    addDetailItem('Frames Analyzed', `${details.frames_analyzed}/${details.total_frames || '?'}`);
  }
...
function showError(error) {
  ...
  if (error.includes('405') || error.includes('404')) {
    errorText = 'The deepfake detection API is not available. Make sure the backend server is running on port 8000.';
  } else if (error.includes('DRM')) {
    errorText = 'This media is protected by DRM and cannot be analyzed. Try downloading it first if permitted.';
  } else if (error.includes('too large')) {
    errorText = 'The file is too large. Maximum file size is 500MB.';
  } else if (error.includes('CORS')) {
    errorText = 'Unable to download this media due to website restrictions. Try uploading it directly.';
  }
