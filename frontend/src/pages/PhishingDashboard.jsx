...
const CATEGORY_COLORS = {
  high_risk: { bg: "bg-red-500/15", border: "border-red-500/30", text: "text-red-400", badge: "bg-red-500/20 text-red-300" },
  medium_risk: { bg: "bg-amber-500/15", border: "border-amber-500/30", text: "text-amber-400", badge: "bg-amber-500/20 text-amber-300" },
  low_risk: { bg: "bg-yellow-500/15", border: "border-yellow-500/30", text: "text-yellow-400", badge: "bg-yellow-500/20 text-yellow-300" },
  safe: { bg: "bg-green-500/15", border: "border-green-500/30", text: "text-green-400", badge: "bg-green-500/20 text-green-300" },
};
...
function RiskBadge({ score, category }) {
  ...
  return (
    <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold ${colors.badge}`}>
      <div className={`w-2 h-2 rounded-full ${score >= 70 ? "bg-red-400" : score >= 40 ? "bg-amber-400" : "bg-green-400"}`} />
      {score}/100
    </div>
  );
}
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <p className="text-white font-bold text-sm truncate mb-1">{threat.url}</p>
      ...
      {threat.reasons?.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-1.5">
          {threat.reasons.slice(0, 2).map((r, i) => (
            <span key={i} className="text-white/50 text-xs bg-white/5 px-2 py-0.5 rounded-md">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className={`${colors.bg} border ${colors.border} rounded-xl p-4 mb-4`}>
          <p className="text-white/50 text-xs font-bold uppercase tracking-wider mb-1">URL</p>
          <p className="text-white font-mono text-sm break-all">{threat.url}</p>
        </div>

        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="bg-slate-800/50 rounded-lg p-3 text-center">
            <p className={`text-2xl font-black ${colors.text}`}>{threat.riskScore}</p>
            <p className="text-white/40 text-xs font-semibold">Risk Score</p>
          </div>
          <div className="bg-slate-800/50 rounded-lg p-3 text-center">
            <p className="text-2xl font-black text-blue-400">{(threat.confidence * 100).toFixed(0)}%</p>
            <p className="text-white/40 text-xs font-semibold">Confidence</p>
          </div>
          <div className="bg-slate-800/50 rounded-lg p-3 text-center">
            <p className={`text-sm font-black ${colors.text} capitalize`}>{threat.category?.replace("_", " ")}</p>
            <p className="text-white/40 text-xs font-semibold mt-1">Category</p>
          </div>
        </div>

        <div className="mb-4">
          <p className="text-white/50 text-xs font-bold uppercase tracking-wider mb-3">Why this was flagged</p>
...
export default function PhishingDashboard() {
  ...
  const [stats, setStats] = useState({ total: 0, highRisk: 0, mediumRisk: 0, lowRisk: 0, today: 0 });
  ...
  async function fetchThreats() {
    try {
      ...
      setError("Cannot connect to PhishGuard API. Make sure the backend is running on port 5000.");
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-white/50 font-medium">Real-time phishing threat monitoring and URL analysis</p>
          </div>
          <Link
            to="/login"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <StatCard value={stats.total} label="Total Threats" color="text-blue-400" icon="🎯" />
          <StatCard value={stats.today} label="Detected Today" color="text-purple-400" icon="📅" />
          <StatCard value={stats.highRisk} label="High Risk" color="text-red-400" icon="🔴" />
          <StatCard value={stats.mediumRisk} label="Medium Risk" color="text-amber-400" icon="🟡" />
        </div>
        ...
        <div className="bg-slate-800/50 border border-white/10 rounded-2xl p-6 mb-6 backdrop-blur-sm">
          ...
          {testResult && !testResult.error && (
            <div className={`mt-4 p-4 rounded-xl border ${testResult.safe
              ? "bg-green-500/10 border-green-500/30"
              : "bg-red-500/10 border-red-500/30"
              }`}>
              <div className="flex items-center justify-between mb-2">
                <span className={`font-bold ${testResult.safe ? "text-green-400" : "text-red-400"}`}>
                  {testResult.safe ? "✅ URL appears safe" : "⚠️ Potential phishing detected!"}
                </span>
                <RiskBadge score={testResult.riskScore} category={testResult.category} />
              </div>
          ...
          <div className="flex gap-2">
            {["all", "high_risk", "medium_risk", "low_risk"].map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-colors ${filter === f
        ...
        {error && (
          <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 mb-6">
            ...
            <p className="text-white/40 text-xs mt-1">Run: cd .wingineers/backend && npm start</p>
          </div>
        )}
        ...
        <div className="mb-4">
          ...
          {loading ? (
            <div className="text-center py-12">
              ...
              <p className="mt-3 text-white/40 font-medium">Loading threats...</p>
            </div>
          ) : threats.length === 0 ? (
            <div className="text-center py-12 bg-slate-800/30 rounded-2xl border border-white/5">
              ...
              <p className="text-white/30 text-sm mt-1">
                Threats will appear here when the browser extension detects suspicious links.
                <br />Try testing a URL above!
              </p>
            </div>
          ) : (
            <div>
              {threats.map((threat) => (
                <ThreatRow key={threat.id} threat={threat} onClick={setSelectedThreat} />
              ))}
            </div>
          )}
        </div>
